...
import owesMeCommand from '../../src/commands/utility/owes-me.js';

describe('Owes Me Command', () => {
    let mockInteraction;
    ...
    describe('Command Data', () => {
        test('should have correct command structure', () => {
            expect(owesMeCommand.data.name).toBe('owes-me');
            expect(owesMeCommand.data.description).toBe('Check how much someone owes you');
        });
    });
    ...
    describe('Execute Function', () => {
        test('should reject self-debt', async () => {
            ...
            await owesMeCommand.execute(mockInteraction);
        ...
        test('should display debt with no transactions', async () => {
            ...
            expect(getTotalDebtFromSomeone).toHaveBeenCalledWith('user123', 'debtor456');
            expect(mockInteraction.reply).toHaveBeenCalledWith({
                content: '<@debtor456> owes you $0 ',
                flags: MessageFlags.Ephemeral
            });
        });
        ...
        test('should display debt with single transaction', async () => {
            ...
            expect(mockInteraction.reply).toHaveBeenCalledWith({
                content: '<@debtor456> owes you $25.5 from 1 transaction',
                flags: MessageFlags.Ephemeral
            });
        });
        ...
        test('should display debt with multiple transactions', async () => {
            ...
            expect(mockInteraction.reply).toHaveBeenCalledWith({
                content: '<@debtor456> owes you $150.75 from 5 transactions',
                flags: MessageFlags.Ephemeral
            });
        });
        ...
        test('should handle database errors', async () => {
            ...
            await expect(owesMeCommand.execute(mockInteraction)).rejects.toThrow('Database error');
        });
    });
